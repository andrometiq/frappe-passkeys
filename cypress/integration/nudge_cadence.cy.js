...
const wait_for_nudge_record = () =>
	cy.wait("@record_nudge", { timeout: 20000 }).its("response.statusCode").should("be.within", 200, 299);
...
chromium_only("passkey enrollment nudge — cadence", () => {
	...
	it("'Not now' records a server-side decline", () => {
		cy.intercept_frappe_method("passkeys.passkey.record_nudge", "record_nudge");
		cy.login(USER, PW());
		cy.visit_desk(USER);
		wait_for_nudge_record(); // shown
		cy.get(".modal-dialog", { timeout: 20000 }).contains("Not now").click();
		wait_for_nudge_record(); // declined
