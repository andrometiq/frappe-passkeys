...
chromium_only("passkey management — User form cards", () => {
	...
	it("renders a card per credential with an accessible label + Synced/Device-bound badge", () => {
		...
		cy.get(".passkey-card .passkey-rename").should("have.attr", "aria-label").and("match", /^Rename passkey /);
		cy.get(".passkey-card .passkey-delete").should("have.attr", "aria-label").and("match", /^Delete passkey /);
	});

	it("renames a credential inline (display-only, no sudo prompt)", () => {
		cy.visit("/app/user/" + USER);
		cy.get(".passkey-card .passkey-rename", { timeout: 20000 }).first().click();
		cy.get(".modal-dialog input[data-fieldname='label'], .modal-dialog input")
			.first()
			.clear()
			.type("My laptop");
		cy.get(".modal-dialog .btn-primary").contains("Save").click();
		cy.get(".passkey-card .passkey-card-label", { timeout: 20000 }).should("contain.text", "My laptop");
	});
});
