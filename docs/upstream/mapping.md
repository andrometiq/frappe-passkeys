...
| `passkeys/confirm.py` — `begin_confirmation` / `verify_confirmation` / `reauth_password` + `@passkey_protected` (DESIGN §7; **built** — §16 A51 canonical namespace, A53 password leg) | `frappe/passkey.py` (public re-auth API) | fold |
...
| `passkeys/notifications.py` — out-of-band change emails (add/remove/disable/flag, §8.3) + risk-event Activity-Log telemetry (§8.5) + admin-change owner notices (§8.6); **built** | `frappe/passkey.py` (bodies stay inline `_()`-wrapped, NOT `templates/emails/` — §16 A52) | fold |
| `passkeys/boot.py` — Desk `extend_bootinfo` flag (§8.1) + per-user enrollment-nudge state (`__passkeys` Defaults rows, §2.3/§8.4) + `record_nudge_event` (§16 A57); **built** | `frappe/passkey.py` + core boot registration in `frappe/boot.py` / `frappe/hooks.py` | fold |
| §11 dormant-shell layer — `install.is_core_native`/`dormant` (`find_spec("frappe.passkey")`, memoized), `session.refuse_if_core_native` → 417 `PasskeyServedByCore` on all 18 whitelisted endpoints, all 7 hook entry points no-op (§16 A43/A56); **built** | — | **discard** (upgrade scaffolding whose whole job is to no-op *on* a passkey-native core; gone once core is native) |
| `passkeys/aaguid.py` + `passkeys/public/aaguid-map.json` — vendored community AAGUID→provider snapshot + loader (§8.2; frappe-free / webauthn-free, `lru_cache`); **built** | `frappe/passkey.py` loader + `frappe/passkey/…` static asset | fold (loader) + verbatim (JSON asset moves) |
...
| `passkeys/shims/portal_nudge.py` (`update_website_context` → the self-gating portal bundle + a `frappe.boot.passkeys` bridge on **every** authenticated website render — website boot omits `extend_bootinfo`; §5.1/§8.4); **built** | native portal boot + `web_include_js` slot | **shim** (deleted — website nudge becomes native) |
...
| `passkeys/public/js/passkey_manage_common.js` (shared Desk+portal management logic: banners, origins mirror, RP-ID one-way-door revert — C8) | `frappe/public/js/frappe/utils/` + passkey bundle | fold |
...
| `passkeys/www/passkeys.{html,py}` portal management page (§5.1/§8.4); **built** | `frappe/www/passkeys.{html,py}` | verbatim |
