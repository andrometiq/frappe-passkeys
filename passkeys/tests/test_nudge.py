...
"""Enrollment-nudge cadence machinery (DESIGN-v1 §8.4, knobs per pass-3 F3-8):
...
class NudgeCadenceTest(IntegrationTestCase):
	...
	# ---- cap (max_prompts, knob-referenced per F3-8) ----------------------
	...
	# ---- cooldown (cooldown_days, knob-referenced per F3-8) ---------------
	...
	def test_declined_refreshes_cooldown_without_advancing_cap(self):
		...
		state = boot.get_nudge_state(user)
		self.assertEqual(state["declines"], 0)  # A-F24: `shown` is the counter, not `declined`
