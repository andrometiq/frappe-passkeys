...
"""Registration ceremony endpoints (DESIGN-v1 §3.2). Authenticated,
sudo-gated, JSON bodies only.

``webauthn`` (via ``passkeys.engine``) is imported **lazily inside the endpoint
bodies** (§1.3): this module sits on no hook path, but the discipline keeps a
...
def begin_registration(flow: str = "explicit"):
	"""Issue creation options + cache the challenge (§3.2). Identity is
...
def verify_registration(state_id: str, credential, label: str | None = None):
	...
	result = engine.verify_registration(
		...
		require_user_verification=False,  # §3.7 matrix layered below, not at the library
	)
...
def _require_any_login_mode(settings) -> None:
	"""Registration is gated on ANY passkey login mode (§3.0): a 2FA-only site's
...
def _require_sudo_for_registration(settings, user: str, flow: str) -> None:
	...
	if flow == "conditional_create":
		# The silent upgrade rides the just-typed password's freshness (§7.1).
	...
	# explicit add
	if seeded_by == "weak":
		# Restricted bootstrap (§7.1 / §16 A7): a weak (email-link / social)
...
def _enforce_request_host(origins: list) -> None:
	"""Fail-closed host membership (§3.1 / §9.2). No-ops when there is no HTTP
