...
core's login page with **zero template fork** (DESIGN-v1 §5.1). Appends the
...
**Hook-path import discipline (§1.3):** this runs on **every** website render,
...
# `frappe.passkeys_common` global the bundle reads (§5.2, frontend notes).
LOGIN_JS = (
...
def _any_login_mode_enabled() -> bool:
	"""True iff `login_with_passkey` or `passkey_as_second_factor` is on (§3.0
	any-mode gate). Webauthn-free settings read; fail-closed on any error."""
	try:
